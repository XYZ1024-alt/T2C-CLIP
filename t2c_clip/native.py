...
try:
    from t2c_clip import _native as native_extension
except ImportError as exc:
    raise ImportError(
        "the mandatory t2c_clip Rust extension is unavailable; install Rust 1.85+ "
...
def _validate_native_extension() -> tuple[str, int]:
    try:
        ...
        raise ImportError(
            "t2c_clip._native is missing its ABI metadata; rebuild the package with `uv sync`"
        ) from exc
    if int(abi_version) != EXPECTED_NATIVE_ABI_VERSION:
        raise ImportError(
            "t2c_clip native ABI mismatch: "
